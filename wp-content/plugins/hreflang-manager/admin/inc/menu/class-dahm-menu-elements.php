...
class Dahm_Menu_Elements {
	 ...
	 * The constructor.
	 *
	 * @param Dahm_Shared $shared An instance of the shared class.
	...
	public function display_list_table(
		...
		if ( ! is_null( $post_search_input ) && mb_strlen( trim( $post_search_input ) ) > 0 ) {
			...
			$total_items = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}da_hm_$db_table $filter" );
		...
		switch ( $this->menu_slug ) {
			...
			$pag = new dahm_pagination( $this->shared );
		...
		?>
			...
			$results = $wpdb->get_results(
				"SELECT * FROM {$wpdb->prefix}da_hm_$db_table $filter ORDER BY $db_primary_key DESC $query_limit",
				ARRAY_A
			);
			// phpcs:enable
			...
			<?php if ( count( $results ) > 0 ) : ?>
				...
				<div class="dahm-crud-table">

					<!-- list of tables -->
					<table class="dahm-crud-table__daext-items">
						<thead>
						<tr>
							<th>
								<input type="checkbox" class="dahm-cb-select-all">
							</th>
							<?php
						...
						<?php foreach ( $results as $result ) : ?>
							<tr>
								<td><input type="checkbox"
											class="dahm-bulk-action-checkbox"
								...
								foreach ( $columns as $key => $column ) {
									...
									<td>
										...
										<?php if ( 0 === $key ) : ?>
											<a class="dahm-crud-table__item-name" href="admin.php?page=<?php echo esc_attr( $this->shared->get( 'slug' ) ); ?>_<?php echo esc_attr( $page_slug ); ?>&edit_id=<?php echo esc_attr( $result[ $db_primary_key ] ); ?>">
												<?php echo esc_html( stripslashes( $result[ $column['db_field'] ] ) ); ?>
											</a>
											<div class="dahm-crud-table__row-actions">
											<div class="dahm-crud-table__row-actions-single-action">
												<a href="admin.php?page=<?php echo esc_attr( $this->shared->get( 'slug' ) ); ?>_<?php echo esc_attr( $page_slug ); ?>&edit_id=<?php echo esc_attr( $result[ $db_primary_key ] ); ?>">Edit</a>
											</div>
											<div>&nbsp|&nbsp</div>
											<div class="dahm-crud-table__row-actions-single-action">
												<form method="POST"
													...
													<?php wp_nonce_field( 'dahm_clone_' . $this->menu_slug . '_' . intval( $result[ $db_primary_key ], 10 ), 'dahm_clone_' . $this->menu_slug . '_nonce' ); ?>
													<button type="submit" name="clone_id" value="<?php echo esc_html( $result[ $db_primary_key ] ); ?>">Duplicate</button>
												</form>
											</div>
											<div>&nbsp|&nbsp</div>
											<div class="dahm-crud-table__row-actions-single-action dahm-crud-table__row-actions-single-action-delete">
												<form method="POST"
														action="admin.php?page=<?php echo esc_attr( $this->shared->get( 'slug' ) ); ?>_<?php echo esc_attr( $page_slug ); ?>"
														id="delete-item-<?php echo esc_attr( $result[ $db_primary_key ] ); ?>">
													<?php wp_nonce_field( 'dahm_delete_' . $this->menu_slug . '_' . intval( $result[ $db_primary_key ], 10 ), 'dahm_clone_' . $this->menu_slug . '_nonce' ); ?>
													<input type="hidden" name="delete_id" value="<?php echo esc_html( $result[ $db_primary_key ] ); ?>">
													<button type="submit" value="<?php echo esc_html( $result[ $db_primary_key ] ); ?>"><?php esc_html_e( 'Delete', 'hreflang-manager' ); ?></button>
												</form>
											</div>
										</div>
				...
				<!-- Bulk Actions -->
				<div class="dahm-crud-table-controls">

					<div class="dahm-crud-table-controls__bulk-actions">
						<form method="POST" action="admin.php?page=<?php echo esc_attr( $this->shared->get( 'slug' ) ); ?>_<?php echo esc_attr( $this->slug_plural ); ?>">
							<select name="bulk_action" id="bulk_action">
								<option value=""><?php esc_html_e( 'Bulk actions', 'hreflang-manager' ); ?></option>
								<option value="delete"><?php esc_html_e( 'Delete', 'hreflang-manager' ); ?></option>
							</select>
							<?php wp_nonce_field( 'dahm_bulk_action_' . $this->menu_slug, 'dahm_bulk_action_' . $this->menu_slug . '_nonce' ); ?>
							<input id="bulk-action-selected-items" type="hidden" name="bulk-action-selected-items" value="">
							<input id="dahm-submit-bulk-action" type="submit" class="button dahm-admin-page-button" value="<?php esc_html_e( 'Apply', 'hreflang-manager' ); ?>">
						</form>
					</div>

					<div class="dahm-crud-table-controls__pagination-container">
						<!-- Display the pagination -->
						<?php if ( $pag->total_items > 0 ) : ?>
							<div class="dahm-crud-table-controls__daext-tablenav">
									<span class="dahm-crud-table-controls__daext-displaying-num"><?php echo esc_html( $pag->total_items ); ?>&nbsp<?php esc_html_e( 'items', 'hreflang-manager' ); ?></span>
									<?php $pag->show(); ?>
							</div>
						<?php endif; ?>
					</div>
			...
			<?php else : ?>
				...
				if ( mb_strlen( trim( $filter ) ) > 0 ) {
					$this->shared->save_dismissible_notice(
						__( 'There are no results that match your filter.', 'hreflang-manager' ),
						'updated'
					);
				}
				...
				if ( 0 === mb_strlen( trim( $filter ) ) ) {
					echo '<div class="dahm-crud-table__no-items-found-message">' . esc_html__( 'Nothing to show yet! Add some items by clicking the Add New button.', 'hreflang-manager' ) . '</div>';
				}
	...
	public function header_bar() {
		...
		if ( 'new' === $action ) {
			$page_title = __( 'Add New', 'hreflang-manager' ) . ' ' . $this->label_singular;
		} elseif ( null !== $edit_id ) {
			$page_title = __( 'Edit', 'hreflang-manager' ) . ' ' . $this->label_singular;
		} else {
		...
		<div class="dahm-header-bar">

			<div class="dahm-header-bar__left">
				<div class="dahm-header-bar__page-title"><?php echo esc_html( $page_title ); ?></div>
				<?php if ( 'list' === $action && 'crud' === $this->context && null === $edit_id ) : ?>
					<a href="<?php echo esc_url( get_admin_url() . 'admin.php?page=da_hm_' . $this->slug_plural . '&action=new' ); ?>"
						class="dahm-button dahm-header-bar__add-new-button">
						<?php $this->shared->echo_icon_svg( 'plus' ); ?>
						<div class="dahm-header-bar__add-new-button-text"><?php esc_html_e( 'Add New', 'hreflang-manager' ); ?></div>
					</a>
				<?php endif; ?>
			</div>

			<div class="dahm-header-bar__right">
				<?php if ( 'new' === $action || null !== $edit_id ) : ?>
					<a href="#" onclick="document.getElementById('form1').submit()" class="dahm-btn dahm-btn-primary"><?php esc_html_e( 'Save Changes', 'hreflang-manager' ); ?></a>
				<?php endif; ?>
			</div>
	...
	public function new_item() {
		...
		<div class="dahm-main-form">
			...
			<form id="form1" name="form1" method="POST"
				...
				wp_nonce_field( 'dahm_create_update_' . $this->menu_slug, 'dahm_create_update_' . $this->menu_slug . '_nonce' );
	...
	public function edit_item( $edit_id ) {
		...
		<div class="dahm-main-form">
			...
			<form id="form1" name="form1" method="POST"
				...
				// Sanitize the field name.
				$db_table_name = $wpdb->prefix . 'da_hm_' . $this->db_table;
	...
	public function section_header( $label, $section_id, $icon_id = null ) {
		...
		<div class="dahm-main-form__section-header group-trigger" data-trigger-target="<?php echo esc_attr( $section_id ); ?>">
			<div class="dahm-main-form__section-header-title">
				<?php $this->shared->echo_icon_svg( $icon_id ); ?>
				<div class="dahm-main-form__section-header-title-text"><?php echo esc_html( $label ); ?></div>
			</div>
			<div class="dahm-main-form__section-header-toggle">
	...
	public function input_field(
		...
		<div class="dahm-main-form__daext-form-field" valign="top" data-section-id="<?php echo esc_attr( $section_id ); ?>">
			<div><label for="title"><?php echo esc_html( $label ); ?><?php echo $required ? ' <span class="dahm-required">*</span>' : ''; ?></label></div>
			<div>
	...
	public function display_admin_toolbar() {
		...
		<div class="dahm-admin-toolbar">
			<div class="dahm-admin-toolbar__left-section">
				<div class="dahm-admin-toolbar__menu-items">
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=da_hm_connections' ) ); ?>" class="dahm-admin-toolbar__plugin-logo">
					...
					foreach ( $this->config['admin_toolbar']['items'] as $key => $item ) {
						...
						<a href="<?php echo esc_attr( $item['link_url'] ); ?>" class="dahm-admin-toolbar__menu-item <?php echo 'dahm-' . $this->menu_slug === $item['menu_slug'] ? 'is-active' : ''; ?>">
							<div class="dahm-admin-toolbar__menu-item-wrapper">
								<?php $this->shared->echo_icon_svg( $item['icon'] ); ?>
								<div class="dahm-admin-toolbar__menu-item-text"><?php echo esc_html( $item['link_text'] ); ?></div>
							</div>
						</a>
					...
					<div class="dahm-admin-toolbar__menu-item dahm-admin-toolbar__menu-item-more">
						<div class="dahm-admin-toolbar__menu-item-wrapper">
							<?php $this->shared->echo_icon_svg( 'grid-01' ); ?>
							<div class="dahm-admin-toolbar__menu-item-text"><?php esc_html_e( 'More', 'hreflang-manager' ); ?></div>
							<?php $this->shared->echo_icon_svg( 'chevron-down' ); ?>
						</div>
						<ul class="dahm-admin-toolbar__pop-sub-menu">
							...
							foreach ( $this->config['admin_toolbar']['more_items'] as $key => $more_item ) {
								...
								<li>
									<a href="<?php echo esc_attr( $more_item['link_url'] ); ?>">
										<?php echo '<div class="dahm-admin-toolbar__more-item-item-text">' . esc_html( $more_item['link_text'] ) . '</div>'; ?>
										<?php

										if ( true === isset( $more_item['pro_badge'] ) && $more_item['pro_badge'] ) {
											echo '<div class="dahm-admin-toolbar__pro-badge">' . esc_html__( 'PRO', 'hreflang-manager' ) . '</div>';
										}
			...
			<div class="dahm-admin-toolbar__right-section">
				<!-- Display the upgrade button in the Free version. -->
				<?php if ( constant( 'DAHM_EDITION' ) === 'FREE' ) : ?>
				<a href="https://daext.com/hreflang-manager/" target="_blank" class="dahm-admin-toolbar__upgrade-button">
					<?php $this->shared->echo_icon_svg( 'diamond-01' ); ?>
					<div class="dahm-admin-toolbar__upgrade-button-text"><?php esc_html_e( 'Unlock Extra Features with HM Pro', 'hreflang-manager' ); ?></div>
				</a>
				<?php endif; ?>
				<a href="https://daext.com" target="_blank" class="dahm-admin-toolbar__daext-logo-container">
				<img class="dahm-admin-toolbar__daext-logo" src="<?php echo esc_url( $this->shared->get( 'url' ) . 'admin/assets/img/daext-logo.svg' ); ?>" alt="DAEXT" />
				</a>
			</div>
		</div>
	...
	public function display_pro_features() {
		...
		<div class="dahm-admin-body">
			...
			<div class="dahm-pro-features">

				<div class="dahm-pro-features__wrapper">

					<div class="dahm-pro-features__left">
						<div class="dahm-pro-features__title">
							<div class="dahm-pro-features__title-text"><?php esc_html_e( 'Unlock Advanced Features with Hreflang Manager Pro', 'hreflang-manager' ); ?></div>
							<div class="dahm-pro-features__pro-badge"><?php esc_html_e( 'PRO', 'hreflang-manager' ); ?></div>
						</div>
						<div class="dahm-pro-features__description">
							<?php
							esc_html_e(
								'Sync the hreflang data between different websites, bulk import hreflang data from a spreadsheet, configure up to 100 alternate versions of a page, optionally add hreflang directly in the post editor, and more!',
								'hreflang-manager'
							);
							?>
						</div>
						<div class="dahm-pro-features__buttons-container">
							<a class="dahm-pro-features__button-1" href="https://daext.com/hreflang-manager/" target="_blank">
								<div class="dahm-pro-features__button-text">
									<?php esc_html_e( 'Learn More', 'hreflang-manager' ); ?>
								</div>
								<?php $this->shared->echo_icon_svg( 'arrow-up-right' ); ?>
							</a>
							<a class="dahm-pro-features__button-2" href="https://daext.com/hreflang-manager/#pricing" target="_blank">
								<div class="dahm-pro-features__button-text">
									<?php esc_html_e( 'View Pricing & Upgrade', 'hreflang-manager' ); ?>
								</div>
								<?php
					...
					<div class="dahm-pro-features__right">
						...
						foreach ( $pro_features_data_a as $key => $pro_feature_data ) {
							...
							<div class="dahm-pro-features__single-feature">
								<div class="dahm-pro-features__single-feature-wrapper">
									<?php $this->shared->echo_icon_svg( $pro_feature_data['icon'] ); ?>
									<div class="dahm-pro-features__single-feature-name">
				...
				<div class="dahm-pro-features__footer-wrapper">
					<div class="dahm-pro-features__footer-wrapper-inner">
						<div class="dahm-pro-features__footer-wrapper-left">
							<?php esc_html_e( 'Built for WordPress creators by the DAEXT team', 'hreflang-manager' ); ?>
						</div>
						<a class="dahm-pro-features__footer-wrapper-right" href="https://daext.com/products/" target="_blank">
							<div class="dahm-pro-features__footer-wrapper-right-text">
								<?php esc_html_e( 'More Tools from DAEXT', 'hreflang-manager' ); ?>
							</div>
							<?php $this->shared->echo_icon_svg( 'arrow-up-right' ); ?>
						</a>
					</div>
	...
	public function handle_duplicate() {
		...
		if ( ! is_null( $data['clone_id'] ) ) {
			...
			// Nonce verification.
			check_admin_referer( 'dahm_clone_' . $this->menu_slug . '_' . $data['clone_id'], 'dahm_clone_' . $this->menu_slug . '_nonce' );

			$this->duplicate_record( $this->db_table, $this->primary_key, $data['clone_id'] );

			$this->shared->save_dismissible_notice(
				__( 'The item has been successfully duplicated.', 'hreflang-manager' ),
				'updated'
			);
	...
	public function handle_delete() {
		...
		if ( ! is_null( $data['delete_id'] ) ) {
			...
			// Nonce verification.
			check_admin_referer( 'dahm_delete_' . $this->menu_slug . '_' . $data['delete_id'], 'dahm_clone_' . $this->menu_slug . '_nonce' );
			...
			if ( ! $result['is_deletable'] ) {
				...
				if ( false !== $query_result ) {
					$this->shared->save_dismissible_notice(
						__( 'The item has been successfully deleted.', 'hreflang-manager' ),
						'updated'
					);
				}
			}
	...
	public function handle_bulk_actions() {
		...
		if ( 'delete' === $bulk_action ) {
			...
			if ( ! is_null( $delete_id ) ) {
				...
				// Nonce verification.
				check_admin_referer( 'dahm_bulk_action_' . $this->menu_slug, 'dahm_bulk_action_' . $this->menu_slug . '_nonce' );
				...
				if ( isset( $query_result ) && false !== $query_result ) {
					...
					$this->shared->save_dismissible_notice(
						$deleted_items_count . ' ' . __( 'items have been successfully deleted.', 'hreflang-manager' ),
						'updated'
					);
				...
				if ( count( $delete_id_non_deletable ) > 0 ) {
					...
					$this->shared->save_dismissible_notice(
						__( 'The', 'hreflang-manager' ) . ' ' . strtolower( $this->label_plural ) . ' ' . __( "with the following IDs are used in one or more automatic internal links and can't be deleted:", 'hreflang-manager' ) . ' ' . implode( ', ', $delete_id_non_deletable ) . '.',
						'error'
					);
	...
	public function display_crud_menu() {
		...
		<!-- Dialog Confirm -->
		<div id="dialog-confirm" title="<?php esc_attr_e( 'Delete the item?', 'hreflang-manager' ); ?>" class="daext-display-none">
			<p>
			<?php
			esc_html_e(
	...
	public function print_form_fields_from_array( $sections ) {
		...
		foreach ( $sections as $section ) {
			...
			echo '<div class="dahm-main-form__daext-form-section" data-id="' . esc_attr( $section['section_id'] ) . '">';
			...
			echo '<div class="dahm-main-form__daext-form-section-body" data-section-id="' . esc_attr( $section['section_id'] ) . '">';
	...
	public function display_crud_menu_search_form( $post_search_input ) {
		...
		<form action="admin.php" method="GET" id="items-filter">
			...
			<div class="dahm-crud-table-search-form">

				<div id="daext-search-form">

					<input type="hidden" name="page" value="da_hm_<?php echo esc_attr( $this->slug_plural ); ?>">
					<input class="dahm-crud-table-search-form__post-search-input" type="text" name="s"
							value="<?php echo null !== $post_search_input ? esc_attr( stripslashes( $post_search_input ) ) : ''; ?>" autocomplete="off" maxlength="255">
					<input class="button dahm-admin-page-button" type="submit" value="Search <?php echo esc_attr( $this->label_plural ); ?>">
