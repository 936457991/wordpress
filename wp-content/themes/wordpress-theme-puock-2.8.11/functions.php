...
if (!function_exists('pk_paging')) {
    function pk_paging($pnum = 2)
    {
        ...
        if ($paged > $pnum + 2) echo "<li><a href='#' onclick='event.preventDefault();' rel='nofollow'>...</a></li>";
        ...
        if ($paged < $max_page - $pnum - 1) {
            echo "<li><a href='#' onclick='event.preventDefault();' rel='nofollow'>...</a></li>";
            page_link($max_page);
        }
...
function my_tag_archive_h1() {
  if ( is_tag() ) {
    echo '<header class="page-header">';
    echo '<h1 class="page-title">' . single_tag_title( '标签：', false ) . '</h1>';
    echo '</header>';
  }
}
add_action( 'wp_body_open', 'my_tag_archive_h1' );
