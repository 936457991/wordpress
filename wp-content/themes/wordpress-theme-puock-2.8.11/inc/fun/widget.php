...
class puockReadPerson extends puockWidgetBase {
    ...
    public function update($cur,$old){
        pk_cache_delete(PKC_WIDGET_LINK_PERSONS);
        return $cur;
    }

    function widget( $args, $instance ){
        global $wpdb;
        $links = pk_cache_get(PKC_WIDGET_LINK_PERSONS);
        if(!$links){
            ...
            $links = $wpdb->get_results($sql2);
            pk_cache_set(PKC_WIDGET_LINK_PERSONS, $links);
        }
        $this->get_common_widget_header($instance); ?>
            ...
            <?php foreach ($links as $link): ?>
             <div class="col col-12 col-lg-6 pl-0">
                 <div class="p-2 text-truncate text-nowrap">
                    <a href="<?php echo empty($link->friend_link_url) ? 'javascript:void(0)':$link->friend_link_url ?>" class="a-link"
                        <?php echo empty($link->friend_link_url) ? '':'target="_blank"' ?>>
                        <span class="t-sm"><?php echo $link->friend_link_name?></span>
                    </a>
                </div>
            </div>
            <?php endforeach; ?>
