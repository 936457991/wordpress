...
function pk_get_main_menu($mobile = false)
{
    ...
    if (is_user_logged_in()) {
        ...
        if (pk_is_checked('open_quick_login')) {
            ...
            $out .= '<li><a data-no-instant data-bs-toggle="tooltip" title="登入" data-title="登入" href="#" onclick="event.preventDefault();" rel="nofollow" class="pk-modal-toggle" data-once-load="true" data-url="' . $url . '"><i class="fa fa-right-to-bracket"></i></a></li>';
        }
    }
    if (!$mobile) {
        if (pk_is_checked('theme_mode_s')) {
            $out .= '<li><a class="colorMode" data-bs-toggle="tooltip" title="模式切换" href="#" onclick="event.preventDefault();" rel="nofollow" ><i class="fa fa-' . ((pk_theme_mode() === 'auto' ? 'circle-half-stroke' : (pk_theme_light() ? 'sun' : 'moon'))) . '"></i></a></li>';
        }
        $out .= '<li><a class="search-modal-btn" data-bs-toggle="tooltip" title="搜索" href="#" onclick="event.preventDefault();" rel="nofollow"><i class="fa fa-search"></i></a></li>';
    }
    $out .= '</ul>';
    return $out;
}
